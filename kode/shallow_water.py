...
2D gruntvannsligninger uten bunnfriksjon:
    ∂_t h   + ∂_x (hu)                  + ∂_y (hv)                  = 0
    ∂_t (hu)+ ∂_x (hu^2 + 1/2 g h^2)    + ∂_y (huv)                 = 0
    ∂_t (hv)+ ∂_x (huv)                 + ∂_y (hv^2 + 1/2 g h^2)    = 0

Skjema: Lax–Friedrichs-lignende FTCS i 2D (oppgavens formel).
CFL: dt <= min( dx / max(|u| + sqrt(g h)),  dy / max(|v| + sqrt(g h)) ).

Kjørbar:
    python kode/shallow_water.py --bc periodic --nx 201 --ny 201 --nt 600

Figurer lagres til rapport/figs/.
"""
