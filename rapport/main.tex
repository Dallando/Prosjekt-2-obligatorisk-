...
% --- Grafikkoppsett ---
\graphicspath{{figs/}} % ser etter bilder i rapport/figs/
\newcommand{\maybefig}[2]{%
  \IfFileExists{#1}{\includegraphics[width=0.8\textwidth]{#1}}{%
    \fbox{\parbox{0.8\textwidth}{\centering \vspace{1em}%
    \textbf{(Mangler figur: #1)}\\
    Kjør Python-skriptet som genererer denne figuren, eller last opp #1 til rapport/figs/. \vspace{1em}}}%
  }%
}
...
\section{Innledning}
Dette prosjektet er todelt. I første del modelleres spredningen av en smittsom sykdom ved hjelp av en SIRS-modell (Susceptible–Infected–Recovered–Susceptible). Systemet løses både deterministisk ved ordinære differensialligninger og stokastisk ved Monte Carlo-simulering. I andre del brukes de numeriske metodene fra kurset til å løse gruntvannsligningene i to dimensjoner med et eksplisitt Lax--Friedrichs-skjema.
...
\end{aligned}
\]
der $a$ er smittetakten, $b$ friskhetsraten, $c$ tap av immunitet, og $N=S+I+R$ er total populasjon. Ved å sette $S' = I' = R' = 0$ fås likevektene
\[
S^*=\frac{b}{a}, \qquad
...
\]
der $k_1=f(t_n,y_n)$ osv. Parametrene er $a=1.0$, $c=0.1$ og $b\in\{\tfrac14,\tfrac12,\tfrac34,1\}$ med startverdier $S_0=300$, $I_0=100$, $R_0=0$ og $N=400$.

\begin{figure}[!ht]
\centering
\maybefig{sirs_ode.png}{Utviklingen av $S$, $I$ og $R$ fra ODE-modellen for ulike verdier av $b$.}
\caption{Utviklingen av $S$, $I$ og $R$ fra ODE-modellen for ulike verdier av $b$.}
\end{figure}

\subsection{Monte Carlo-simulering (Del 1c)}
For å inkludere stokastiske effekter simuleres overganger i hvert lille tidssteg $\Delta t$ med sannsynligheter
\[
P(S\to I)=\frac{aSI}{N}\Delta t, \qquad
P(I\to R)=bI\Delta t, \qquad
P(R\to S)=cR\Delta t,
\]
der $\Delta t = \min\!\bigl(4/(aN),\,1/(bN),\,1/(cN)\bigr)$. Simuleringen repeteres $M$ ganger, og middel- og standardavvik beregnes.

\begin{figure}[!ht]
\centering
\maybefig{sirs_mc.png}{Gjennomsnittlig antall smittede (MC) med $\pm1$ standardavvik.}
...
\end{aligned}
\]
Dette gjør at totalbefolkningen $N(t)$ ikke er konstant. Systemet løses med samme RK4-metode, men med adaptivt tidssteg.

\begin{figure}[!ht]
\centering
\maybefig{sirs_vital.png}{Utviklingen av $I(t)$ med vitaldynamikk.}
\caption{Utviklingen av $I(t)$ med vitaldynamikk.}
\end{figure}

\subsection{Diskusjon}
For små $b$ (langsomme tilfriskninger) vokser smitten og når en endemisk likevekt. Når $b\ge a$ blir $I^*=0$, og sykdommen dør ut. Monte Carlo-modellen viser stokastiske svingninger rundt ODE-likevekten, mens vitaldynamikken innfører langsiktig stabilitet mot en bærekraftig befolkningsstørrelse.
...
\begin{figure}[!ht]
\centering
\maybefig{shallow_init.png}{Initial vannhøyde for dam-brudd-testen.}
\caption{Initial vannhøyde for dam-brudd-testen.}
\end{figure}

\begin{figure}[!ht]
\centering
\maybefig{shallow_periodic_tend.png}{Sluttstadiet for periodiske randbetingelser.}
...
\begin{figure}[!ht]
\centering
\maybefig{shallow_neumann_tend.png}{Bølgespredning i stor skala med Neumann-randbetingelser.}
\caption{Bølgespredning i stor skala med Neumann-randbetingelser.}
\end{figure}

\subsection{Diskusjon}
Skjemaet bevarer massen godt og er stabilt for CFL $\approx 0.45$. Numerisk diffusjon demper høye frekvenser, men gir jevn bølgespredning. Ved Dirichlet-rand observeres refleksjon, mens Neumann tillater fri utstrømning.

\section{Konklusjon}
Prosjektet illustrerer forskjeller mellom deterministiske og stokastiske modeller, samt hvordan eksplisitte metoder kan brukes på PDE-systemer. SIRS-modellen viser hvordan smitteparametere styrer likevekten, mens gruntvannseksperimentet viser bølgedynamikk og numerisk stabilitet under ulike randbetingelser.

\section*{Reproduserbarhet}
Kjør følgende for å generere figurene:
\begin{verbatim}
...
\bibliography{refs} % legg inn refs.bib, ellers får du "empty thebibliography" (ufarlig)
\end{document}
